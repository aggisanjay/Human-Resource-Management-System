...
export default function Sidebar() {
  ...
  return (
    <div className="w-60 h-full bg-white border-r p-4">
      <div className="mb-6 font-bold text-lg">HRMS</div>
      <nav className="flex flex-col gap-2">
        {items.map(i => (
          <Link key={i.to} to={i.to} className={`flex items-center gap-3 px-3 py-2 rounded ${loc.pathname === i.to ? 'bg-gray-100 font-medium' : 'text-gray-600'}`}>
